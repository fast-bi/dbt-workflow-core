...
def log_and_print(message, level="INFO", color=None):
    ...
    with open("dbt_tests_completion.log", "a") as f:
...
def run_dbt_coverage(path_filters, threshold=0.9):
    ...
    try:
        log_and_print(f"Running dbt-coverage test with filters: {path_filters}", "INFO", Fore.CYAN)
        cmd = f"dbt-coverage compute test {path_filters} --cov-format markdown --cov-fail-under {threshold} --cov-report {output_file}"
        ...
        try:
            ...
            with open("dbt_tests_completion.log", "a") as f:
                f.write(e.stdout)
            log_and_print(f"Test coverage is below the threshold of {threshold*100}%", "ERROR", Fore.RED)
...
def analyze_coverage_data(coverage_data, threshold=0.9):
    ...
    log_and_print("\nTest Coverage Summary", "STATUS", Fore.BLUE)
        ...
        if coverage_pct < 1.0:
            ...
            if missing_cols:
                models_needing_fixes.append({
                    ...
                    'untested_columns': missing_cols
                })
    
    models_needing_fixes.sort(key=lambda x: x['coverage'])
    
    if models_needing_fixes:
        log_and_print("\nModels Requiring Test Coverage Improvements", "STATUS", Fore.YELLOW)
        ...
        for item in models_needing_fixes:
            ...
            log_and_print("Columns without tests:", "INFO")
            for col in item['untested_columns']:
                log_and_print(f"  - {col}", "INFO")
    
    if total_coverage < threshold:
        log_and_print(f"\nTest coverage ({total_coverage*100:.1f}%) is below the threshold of {threshold*100}%", "ERROR", Fore.RED)
        return 1
    else:
        log_and_print(f"\nTest coverage ({total_coverage*100:.1f}%) meets or exceeds the threshold of {threshold*100}%", "SUCCESS", Fore.GREEN)
        return 0

def main():
    # Create or clear the log file
    with open("dbt_tests_completion.log", "w") as f:
        f.write("")
    
    threshold = float(os.environ.get('TEST_COVERAGE_THRESHOLD', 0.9))
